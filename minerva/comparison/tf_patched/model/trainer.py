...
# Use TensorFlow's logsumexp instead of scipy
import tensorflow as tf
import tensorflow.compat.v1 as tf_v1
tf_v1.disable_v2_behavior()

def lse(x, axis=None):
    """TensorFlow implementation of logsumexp"""
    return tf_v1.reduce_logsumexp(x, axis=axis)
...
class Trainer(object):
    def __init__(self, params):
        ...
        self.optimizer = tf_v1.train.AdamOptimizer(self.learning_rate)


    def calc_reinforce_loss(self):
        loss = tf_v1.stack(self.per_example_loss, axis=1)  # [B, T]

        self.tf_baseline = self.baseline.get_baseline_value()
        # self.pp = tf_v1.Print(self.tf_baseline)
        # multiply with rewards
        final_reward = self.cum_discounted_reward - self.tf_baseline
        # reward_std = tf_v1.sqrt(tf_v1.reduce_mean(tf_v1.square(final_reward))) + 1e-5 # constant addded for numerical stability
        reward_mean, reward_var = tf_v1.nn.moments(final_reward, axes=[0, 1])
        # Constant added for numerical stability
        reward_std = tf_v1.sqrt(reward_var) + 1e-6
        final_reward = tf_v1.div(final_reward - reward_mean, reward_std)

        loss = tf_v1.multiply(loss, final_reward)  # [B, T]
        self.loss_before_reg = loss

        total_loss = tf_v1.reduce_mean(loss) - self.decaying_beta * self.entropy_reg_loss(self.per_example_logits)  # scalar

        return total_loss

    def entropy_reg_loss(self, all_logits):
        all_logits = tf_v1.stack(all_logits, axis=2)  # [B, MAX_NUM_ACTIONS, T]
        entropy_policy = - tf_v1.reduce_mean(tf_v1.reduce_sum(tf_v1.multiply(tf_v1.exp(all_logits), all_logits), axis=1))  # scalar
    ...
    def initialize(self, restore=None, sess=None):
        ...
        self.input_path = []
        self.first_state_of_test = tf_v1.placeholder(tf_v1.bool, name="is_first_state_of_test")
        self.query_relation = tf_v1.placeholder(tf_v1.int32, [None], name="query_relation")
        self.range_arr = tf_v1.placeholder(tf_v1.int32, shape=[None, ])
        self.global_step = tf_v1.Variable(0, trainable=False)
        self.decaying_beta = tf_v1.train.exponential_decay(self.beta, self.global_step,
                                                   200, 0.90, staircase=False)
        self.entity_sequence = []

        # to feed in the discounted reward tensor
        self.cum_discounted_reward = tf_v1.placeholder(tf_v1.float32, [None, self.path_length],
                                                    name="cumulative_discounted_reward")



        for t in range(self.path_length):
            next_possible_relations = tf_v1.placeholder(tf_v1.int32, [None, self.max_num_actions],
                                                   name="next_relations_{}".format(t))
            next_possible_entities = tf_v1.placeholder(tf_v1.int32, [None, self.max_num_actions],
                                                     name="next_entities_{}".format(t))
            input_label_relation = tf_v1.placeholder(tf_v1.int32, [None], name="input_label_relation_{}".format(t))
            start_entities = tf_v1.placeholder(tf_v1.int32, [None, ])
        ...
        self.loss_before_reg = tf_v1.constant(0.0)
        ...
        self.prev_state = tf_v1.placeholder(tf_v1.float32, self.agent.get_mem_shape(), name="memory_of_agent")
        self.prev_relation = tf_v1.placeholder(tf_v1.int32, [None, ], name="previous_relation")
        self.query_embedding = tf_v1.nn.embedding_lookup(self.agent.relation_lookup_table, self.query_relation)  # [B, 2D]
        layer_state = tf_v1.unstack(self.prev_state, self.LSTM_layers)
        formated_state = [tf_v1.unstack(s, 2) for s in layer_state]
        self.next_relations = tf_v1.placeholder(tf_v1.int32, shape=[None, self.max_num_actions])
        self.next_entities = tf_v1.placeholder(tf_v1.int32, shape=[None, self.max_num_actions])

        self.current_entities = tf_v1.placeholder(tf_v1.int32, shape=[None,])
        ...
        with tf_v1.variable_scope("policy_steps_unroll") as scope:
            ...
            self.test_state = tf_v1.stack(test_state)
    ...
    def bp(self, cost):
        self.baseline.update(tf_v1.reduce_mean(self.cum_discounted_reward))
        tvars = tf_v1.trainable_variables()
        grads = tf_v1.gradients(cost, tvars)
        grads, _ = tf_v1.clip_by_global_norm(grads, self.grad_clip_norm)
        train_op = self.optimizer.apply_gradients(zip(grads, tvars))
        with tf_v1.control_dependencies([train_op]):  # see https://github.com/tensorflow/tensorflow/issues/1899
            self.dummy = tf_v1.constant(0)
        return train_op
